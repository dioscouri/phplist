 ...
 * @since		1.6
 */
class PhplistQueryElement extends JObject
{
 ...
 * @since		1.6
 */
class PhplistQuery extends JObject
{
	...
	public function select($columns)
	{
		...
		if (is_null($this->_select)) {
			$this->_select = new PhplistQueryElement('SELECT', $columns);
		} else {
	...
	public function from($tables)
	{
		if (is_null($this->_from)) {
			$this->_from = new PhplistQueryElement('FROM', $tables);
		} else {
	...
	public function join($type, $conditions)
	{
		...
		$this->_join[] = new PhplistQueryElement(strtoupper($type) . ' JOIN', $conditions);
	...
	public function where($conditions, $glue='AND')
	{
		if (is_null($this->_where)) {
			...
			$this->_where = new PhplistQueryElement('WHERE', $conditions, "\n\t$glue ");
		} else {
	...
	public function group($columns)
	{
		if (is_null($this->_group)) {
			$this->_group = new PhplistQueryElement('GROUP BY', $columns);
		} else {
	...
	public function having($conditions, $glue='AND')
	{
		if (is_null($this->_having)) {
			$glue = strtoupper($glue);
			$this->_having = new PhplistQueryElement('HAVING', $conditions, "\n\t$glue ");
		} else {
	...
	public function order($columns)
	{
		if (is_null($this->_order)) {
			$this->_order = new PhplistQueryElement('ORDER BY', $columns);
		} else {
