...
class PhplistGrid extends JHTMLGrid
{
	/**
	...
	function order($id)
	{
		JHTML::_('script', Phplist::getName().'.js', 'media/com_phplist/js/');
	...
	public static function pagetooltip( $key, $title='Tip', $id='page_tooltip' )
	{
		...
		$disabled = PhplistConfig::getInstance()->get( $constant."_disabled", '0');
		...
		if ($lang->hasKey($constant) && !$disabled)
		{
			...
			$url = JRoute::_("index.php?option=com_phplist&controller={$view}&view={$view}&task={$task}&key={$key}");
