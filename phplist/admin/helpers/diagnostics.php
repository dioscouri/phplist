...
class PhplistHelperDiagnostics extends DSCHelperDiagnostics 
{
    /**
    ...
    public function checkInstallation() 
    {
        ...
        foreach ($functions as $function)
        {
            if (!$this->{$function}())
            {
                return $this->redirect( JText::_("COM_PHPLIST_".$function."_FAILED") .' :: '. $this->getError(), 'error' );
            }
        }
    }
    ...
    {
        if (Phplist::getInstance()->get( __FUNCTION__, '0' ))
        {
            return true;
        }
    
        $table = '#__phplist_config';
        $definitions = array();
        $fields = array();
    ...
    protected function setCompleted( $fieldname, $value='1' )
    {
        ...
        $config = JTable::getInstance( 'Config', 'PhplistTable' );
